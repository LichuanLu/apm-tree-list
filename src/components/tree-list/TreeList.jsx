...
import { initTreeLevelStack, updateTreeMap, initDataSourceCloneWithRows, convertListToMap, filterFromArray } from './TreeListUtils';
...
export default class TreeList extends React.PureComponent {
    ...
    fetchAndUpdateCurrent() {
        ...
        if (current.children === true || current.children === 'true') {
    ...
    getBreadcrumbView(treeLevelStack, current) {
        ...
        if (treeLevelStack.length !== 1) {
            return (<NavBar leftContent={<span>上一级</span>} mode="dark" onLeftClick={() => this.backToPrevLevel()}>{current.label}
        </NavBar>);
        }
    ...
    render() {
        ...
        <ListView initialListSize={initialListSize} dataSource={listData} renderFooter={() => <span>没有更多数据了</span>} renderRow={({ rowData, rowID }) => (<TreeSelectorItem onChange={() => this.onChange(rowID)} showSelect={showSelect} rowData={rowData} emitter={this.emitter} toChildLevel={() => this.toChildLevel(rowID)}/>)} className={`${prefixCls}-view`} style={{ overflow: 'auto' }} delayTime={showDelay ? 500 : 0}/>
      </div>);
    }
}
