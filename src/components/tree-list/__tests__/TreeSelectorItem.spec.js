...
describe('TreeSelectorItem', () => {
  describe('render', () => {
    ...
    const treeSelectorItem = (<TreeSelectorItem rowData={rowData} emitter={emitter}/>)
    ...
    it('contains radio button if showSelect set true', () => {
      const wrapper = shallow(React.cloneElement(treeSelectorItem, { showSelect: true })).dive()
      expect(wrapper.find(Checkbox).length).toBe(1)
    })

    it('childlevel button show and can be click when have children params', () => {
      const toChildLevelStub = sinon.stub()
      const wrapper = shallow(React.cloneElement(treeSelectorItem, { showSelect: true,
        toChildLevel: toChildLevelStub })).dive()
